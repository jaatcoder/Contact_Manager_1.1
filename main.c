...
#define CONTACTS_FILE "/Users/mubinsompod/Projects/project_1-1/contacts.txt"
...
static void delete_clicked(GtkWidget *button, AppWidgets *widgets) {
    ...
    if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
        ...
        // Get values from the filtered model first
...
// New function to show contacts by group
...
// New function to delete a contact from terminal
...
static void confirm_delete_response(GtkDialog *dialog, int response, gpointer user_data) {
    ...
    if (response == GTK_RESPONSE_ACCEPT) {
        if (delete_contact_from_file(data->name, data->contact)) {
            ...
            // Re-load all contacts from the file instead of trying to manipulate the model
